...
function initAds() {
    ...
    if(main) {
        ...
        adDiv.innerHTML = `<div class="bg-gray-100 border border-gray-200 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm cursor-pointer hover:bg-gray-50"><div class="flex items-center gap-4"><div class="w-10 h-10 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-bold text-lg"><i class="ph-bold ph-lightning"></i></div><div><h4 class="font-bold text-gray-800 text-sm">Turbo Speed</h4><p class="text-xs text-gray-500">Download files 5x faster.</p></div></div><button class="bg-black text-white text-xs font-bold px-4 py-2 rounded-lg">Check Now</button></div>`;
    ...
    setTimeout(() => {
        ...
        div.innerHTML = `<div class="max-w-4xl mx-auto flex justify-between items-center"><div class="flex items-center gap-3"><img src="https://ui-avatars.com/api/?name=VPN&background=000" class="w-10 h-10 rounded-lg shadow"><div class="text-sm font-bold">Secure VPN<p class="text-xs font-normal text-gray-500">Fast & Secure</p></div></div><div class="flex gap-2"><button class="bg-green-600 text-white text-xs font-bold px-4 py-2 rounded-lg">Install</button><button id="closeAd" class="text-gray-400 text-xl"><i class="ph-bold ph-x-circle"></i></button></div></div>`;
...
export async function loadApps(cat='All', search='') {
    ...
    if(!found) grid.innerHTML = '<div class="col-span-full text-center text-gray-400 text-sm py-10">No apps found.</div>';
}
...
window.editApp = async (id) => {
    ...
    if(d.exists()) {
        ...
        document.getElementById('appName').value = data.name; document.getElementById('packageName').value = data.packageName; document.getElementById('apkUrl').value = data.apkUrl; document.getElementById('iconUrl').value = data.iconUrl; document.getElementById('developer').value = data.developer; document.getElementById('category').value = data.category; document.getElementById('size').value = data.size; document.getElementById('version').value = data.version;
        document.getElementById('t_verCode').value = t.verCode||''; document.getElementById('t_minSdk').value = t.minSdk||''; document.getElementById('description').value = data.description||'';
