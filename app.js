...
export async function loadApps(category = 'All', searchQuery = '') {
    ...
    try {
        ...
        if (!hasResults) {
            grid.innerHTML = `<div class="col-span-full text-center text-gray-400 py-10 text-sm">No apps found.</div>`;
        }
    } catch (e) { 
        console.error(e);
        loading.classList.add('hidden');
...
export async function loadAppDetails(id) {
    ...
    try {
        ...
        container.innerHTML = '<div class="text-center py-20 text-red-500 text-sm">Error loading app details.</div>';
    }
}
...
function renderFullDetails(id, app, container) {
    ...
    if(app.screenshots && app.screenshots.trim() !== '') {
        ...
        if (shots.length > 0) {
            screenshotsHtml = `<div class="flex gap-3 overflow-x-auto pb-4 no-scrollbar mb-6 md:mb-8 snap-x snap-mandatory">` + 
                shots.map(url => `<img src="${url.trim()}" onerror="this.style.display='none'" class="h-48 md:h-64 rounded-lg md:rounded-xl shadow-md border bg-gray-50 object-cover snap-center shrink-0">`).join('') + 
                `</div>`;
        }
    }
    ...
    container.innerHTML = `
        <div class="flex flex-col md:flex-row gap-6 md:gap-8 mb-6 md:mb-8 items-center md:items-start border-b border-gray-100 pb-6 md:pb-8">
            ...
            <div class="text-center md:text-left flex-1">
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-2 md:gap-3">
                    ...
                    <span class="bg-gray-100 px-3 py-1 rounded-lg font-bold text-gray-600 text-xs md:text-sm">${app.size}</span>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="bg-gray-50 px-5 md:px-6 py-3 md:py-4 border-b border-gray-200"><h3 class="font-bold text-gray-900 flex items-center gap-2 text-xs md:text-base"><i class="ph-fill ph-code text-blue-600"></i> Technical Information</h3></div>
...
function generateTechHtml(d) {
    ...
    return `
        <div class="space-y-4 md:space-y-6">
            <div><div class="font-bold text-blue-800 text-[10px] md:text-xs uppercase mb-1 md:mb-2">Build</div>${row('Version Code', d.verCode)}${row('Date', d.date)}${row('Compress', d.compress)}</div>
            <div><div class="font-bold text-blue-800 text-[10px] md:text-xs uppercase mb-1 md:mb-2">SDK</div>${row('Min SDK', d.minSdk)}${row('Target', d.targetSdk)}${row('Compile', d.compileSdk)}</div>
            <div><div class="font-bold text-blue-800 text-[10px] md:text-xs uppercase mb-1 md:mb-2">System</div><div class="text-xs md:text-sm text-gray-600 mb-1"><span class="font-bold">ABI:</span> ${d.abi}</div>${row('Devices', d.devices)}</div>
            ...
            <div class="text-[9px] md:text-[10px] text-gray-500 break-all font-mono bg-gray-50 p-1.5 md:p-2 border rounded">SHA-256: ${d.sha256 || '-'}</div></div>
        </div>
    `;
}

// Global functions for recommended apps and tracking
async function loadRecommendedApps(currentId) { /* logic same as before, omitted for brevity but functionality preserved by context */ 
...
export function initAdmin() {
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            loadAdminList(); // 🔥 THIS FUNCTION IS THE KEY
        } else {
...
async function loadAdminList() {
    ...
    if(!list) return; // Should not happen in admin.html

    list.innerHTML = '<li class="p-6 text-center text-gray-400 text-sm animate-pulse">Loading app library...</li>';

    try {
        // Simple query first to ensure connection works
        const q = query(collection(db, "apps"), orderBy("uploadedAt", "desc"));
        const snapshot = await getDocs(q);
        ...
        if (snapshot.empty) {
            list.innerHTML = '<li class="p-6 text-center text-gray-400 text-sm">No apps found. Start by adding one!</li>';
            return;
        }
        ...
        list.innerHTML = `<li class="p-6 text-center text-red-500 text-sm">Error loading apps: ${error.message}</li>`;
    }
}
...
window.closeSuccessScreen = () => { document.getElementById('successScreen').classList.add('hidden'); resetForm(); }
